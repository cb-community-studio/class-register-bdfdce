...
import timetableFakerFactory from "./timetableFakerFactory";
...
const TimetableFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = timetableFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("timetable").create(elem));
...
export default connect(null, mapDispatch)(TimetableFakerDialogComponent);
