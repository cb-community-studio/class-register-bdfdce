...
const TimetableCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        ...
        setLoading(true);
        try {
            const result = await client.service("timetable").patch(_entity._id, _data);
            props.onHide();
    ...
    const sessionsOptions = sessions.map((elem) => ({ label: elem.courseID, value: elem._id }));

    return (
        <Dialog header="Edit Info" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div role="timetable-edit-dialog-component">
                <div>
            ...
            <div>
                ...
                <MultiSelect value={_entity?.sessionID} options={sessionsOptions} onChange={(e) => setValByKey("sessionID", e.value)} />
            </div>
...
export default connect(null, mapDispatch)(TimetableCreateDialogComponent);
