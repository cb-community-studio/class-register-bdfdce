...
import discountFakerFactory from "./discountFakerFactory";
...
const DiscountFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = discountFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("discount").create(elem));
...
export default connect(null, mapDispatch)(DiscountFakerDialogComponent);
