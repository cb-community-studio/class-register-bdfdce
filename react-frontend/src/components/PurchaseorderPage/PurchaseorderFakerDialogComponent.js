...
import purchaseorderFakerFactory from "./purchaseorderFakerFactory";
...
const PurchaseorderFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = purchaseorderFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("purchaseorder").create(elem));
...
export default connect(null, mapDispatch)(PurchaseorderFakerDialogComponent);
