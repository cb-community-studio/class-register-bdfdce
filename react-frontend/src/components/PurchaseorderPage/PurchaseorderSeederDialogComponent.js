...
import seederData from "./purchaseorder.seeder.json";
...
const PurchaseorderSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("purchaseorder").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(PurchaseorderSeederDialogComponent);
