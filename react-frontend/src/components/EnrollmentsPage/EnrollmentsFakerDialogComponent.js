...
import enrollmentsFakerFactory from "./enrollmentsFakerFactory";
...
const EnrollmentsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = enrollmentsFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("enrollments").create(elem));
...
export default connect(null, mapDispatch)(EnrollmentsFakerDialogComponent);
