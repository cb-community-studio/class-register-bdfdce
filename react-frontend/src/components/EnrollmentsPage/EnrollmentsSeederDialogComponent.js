...
import seederData from "./enrollments.seeder.json";
...
const EnrollmentsSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("enrollments").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(EnrollmentsSeederDialogComponent);
