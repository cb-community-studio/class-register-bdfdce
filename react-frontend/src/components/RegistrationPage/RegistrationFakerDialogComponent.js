...
import registrationFakerFactory from "./registrationFakerFactory";
...
const RegistrationFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = registrationFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("registration").create(elem));
...
export default connect(null, mapDispatch)(RegistrationFakerDialogComponent);
