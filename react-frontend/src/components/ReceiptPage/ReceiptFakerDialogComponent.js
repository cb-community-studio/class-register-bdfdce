...
import receiptFakerFactory from "./receiptFakerFactory";
...
const ReceiptFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = receiptFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("receipt").create(elem));
...
export default connect(null, mapDispatch)(ReceiptFakerDialogComponent);
