...
const SingleStudentsPage = (props) => {
    ...
    useEffect(() => {
        ...
        client
            .service("students")
            .get(urlParams.singleStudentsId, { query: { $populate: ["userID"] }})
            .then((res) => {
                set_entity(res || {});
    ...
    return (
        <div className="col-12 flex flex-column align-items-center">
            <div className="col-10">
                ...
                <p>students/{urlParams.singleStudentsId}</p>
            </div>
            <div className="grid col-10">
                <div className="card w-full">
            <label className="text-sm text-primary">StudentName</label>
                    <div className="ml-3"><p className="m-0 ml-3" >{_entity?.StudentName}</p></div>
                    <label className="text-sm text-primary">contact</label>
                    <div className="ml-3"><p className="m-0 ml-3" >{_entity?.contact}</p></div>
                    <label className="text-sm text-primary">studentType</label>
                    <div className="ml-3"><p className="m-0 ml-3" >{_entity?.studentType}</p></div>
...
export default connect(mapState, mapDispatch)(SingleStudentsPage);
