...
import studentsFakerFactory from "./studentsFakerFactory";
...
const StudentsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = studentsFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("students").create(elem));
...
export default connect(null, mapDispatch)(StudentsFakerDialogComponent);
