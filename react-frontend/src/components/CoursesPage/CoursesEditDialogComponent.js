...
const CoursesCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        let _data = {
            title: _entity.title,
            topic: _entity.topic,
            creditHours: _entity.creditHours,
            price: _entity.price,
        ...
        setLoading(true);
        try {
            const result = await client.service("courses").patch(_entity._id, _data);
            props.onHide();
    ...
    return (
        <Dialog header="Edit Info" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            ...
            <div>
                <p className="m-0">creditHours:</p>
                <InputText className="w-full mb-3" value={_entity?.creditHours} onChange={(e) => setValByKey("creditHours", e.target.value)}  />
            </div>
            <div>
                <p className="m-0">price:</p>
                <InputText className="w-full mb-3" value={_entity?.price} onChange={(e) => setValByKey("price", e.target.value)}  />
            </div>
            <div>
