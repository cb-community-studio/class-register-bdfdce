...
import paymentFakerFactory from "./paymentFakerFactory";
...
const PaymentFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = paymentFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("payment").create(elem));
...
export default connect(null, mapDispatch)(PaymentFakerDialogComponent);
