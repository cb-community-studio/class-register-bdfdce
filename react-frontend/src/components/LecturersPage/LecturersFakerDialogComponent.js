...
import lecturersFakerFactory from "./lecturersFakerFactory";
...
const LecturersFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = lecturersFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("lecturers").create(elem));
...
export default connect(null, mapDispatch)(LecturersFakerDialogComponent);
