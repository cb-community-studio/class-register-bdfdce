...
import sessionsFakerFactory from "./sessionsFakerFactory";
...
const SessionsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = sessionsFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("sessions").create(elem));
...
export default connect(null, mapDispatch)(SessionsFakerDialogComponent);
