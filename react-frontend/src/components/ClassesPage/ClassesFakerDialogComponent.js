...
import classesFakerFactory from "./classesFakerFactory";
...
const ClassesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = classesFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("classes").create(elem));
...
export default connect(null, mapDispatch)(ClassesFakerDialogComponent);
