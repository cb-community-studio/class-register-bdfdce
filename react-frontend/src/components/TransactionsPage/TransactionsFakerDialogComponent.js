...
import transactionsFakerFactory from "./transactionsFakerFactory";
...
const TransactionsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = transactionsFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("transactions").create(elem));
...
export default connect(null, mapDispatch)(TransactionsFakerDialogComponent);
